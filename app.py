...
# КЭШИРОВАНИЕ ДВИЖКА ОЧИСТКИ ДАННЫХ ДЛЯ ВЫСОКОЙ СКОРОСТИ ИНТЕРФЕЙСА
...
def internal_show_abc_xyz_page(filtered_df):
    st.title("🧮 Модуль совмещенного ABC/XYZ-анализа")
    ...
    with col1: 
        a_limit = st.slider("Граница группы A (% кумулятивной выручки):", 50, 90, 80, key="abc_sl_1")
    with col2: 
        b_limit = st.slider("Граница группы B (следующие % выручки):", 5, 25, 15, key="abc_sl_2")
        
    # Шаг 1: ABC Анализ по объемам стоимости
    ...
    # Шаг 2: XYZ Анализ по стабильности потребления
    df_xyz_raw = df.groupby(['ОЗМ', 'Источник (Файл)'])['Сумма'].sum().unstack(fill_value=0)
    mean_val = df_xyz_raw.mean(axis=1)
    std_val = df_xyz_raw.std(axis=1, ddof=1 if df_xyz_raw.shape[1] > 1 else 0)
    ...
    if df_xyz_raw.shape[1] <= 1:
        df_xyz['Класс XYZ'] = 'X'
        
    # Соединяем матрицы ABC и XYZ
...
# МОДУЛЬ 2: RFM СЕГМЕНТАЦИЯ (ЗАЩИЩЕННАЯ ОТ ДУБЛИКАТОВ В КВАНТИЛЯХ)
...
if uploaded_files:
    if st.session_state.main_df.empty:
        with st.spinner("⏳ Идёт глубокая Power Query очистка данных... Пожалуйста, подождите."):
            calculated_df, is_merged = power_query_clean_engine(
                uploaded_files, 
                st.session_state.pq_skip_top, 
                st.session_state.pq_merge_headers, 
                st.session_state.pq_remove_footer
            )
    ...
    if not main_df.empty:
        ...
        # Сайдбар управления и глобальной сквозной фильтрации
        st.sidebar.markdown("---")
        st.sidebar.subheader("🧮 Глобальные Фильтры BI-платформы")
        ...
        if page == "🧮 1. Загрузка и очистка данных":
            st.markdown("### 🧮 Панель шагов трансформации (Аналог Power Query)")
            ...
            except Exception as e: st.error(f"Ошибка вывода: {e}")
            ...
            st.info("Раздел No-Code графиков активен и использует оптимизированный датасет.")
            
        elif "3. ABC/XYZ" in page:
            internal_show_abc_xyz_page(active_df_global)
            
        elif "4. RFM" in page:
            internal_show_rfm_page(active_df_global)
else:
