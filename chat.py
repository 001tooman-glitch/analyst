...
# МОДУЛЬ 1: УНИВЕРСАЛЬНЫЙ ABC/XYZ КОНСТРУКТОР
def internal_show_abc_xyz_page(filtered_df):
    ...
    with col_abc2:
        x_limit = st.slider("Граница группы X (KV ≤ %):", 5, 20, 10, key="xyz_sl_1")
        y_limit = st.slider("Граница группы Y (KV ≤ %):", 15, 50, 25, key="xyz_sl_2")
    ...
    except Exception as e: st.error(f"Ошибка матрицы: {e}")
...
# МОДУЛЬ 3: ДВИЖОК ОЧИСТКИ ДАННЫХ RUST CALAMINE
...
# МОДУЛЬ 4: ВЫДЕЛЕННЫЙ ДВИЖОК ОТРИСОВКИ КОНСТРУКТОРА ГРАФИКОВ PLOTLY
...
if uploaded_files:
    ...
    if not main_df.empty:
        ...
        if page == "🗂️ 1. Загрузка и очистка данных":
                ...
                with card_cols[j % len(card_cols)]:
                    ...
                    if t_col != "-- Выберите заголовок --":
                        try:
                            ...
                            st.markdown(f'<div style="background-color:#f8f9fa; border:1px solid #dee2e6; border-radius:10px; padding:20px; text-align:center; margin-bottom:15px;"><div style="color:#6c757d; font-size:15px;">{t_col}</div><div style="color:#1f77b4; font-size:26px; font-weight:bold; margin-top:5px;">{current_val:,.2f}</div></div>', unsafe_allow_html=True)
                        except: pass
            ...
            for i in range(st.session_state.manual_charts):
                ...
                if x_ax != "-- Выберите заголовок --" and y_ax != "-- Выберите заголовок --":
                    render_custom_chart(active_df_global, x_ax, y_ax, style, color, lbl, f_format, f_round, f_size, f_color, f_pos, horiz, rot, i)
        ...
        elif page == "🧮 3. ABC/XYZ-аналитика ОЗМ": internal_show_abc_xyz_page(active_df_global)
        elif page == "👥 4. RFM-сегментация": internal_show_rfm_page(active_df_global)
